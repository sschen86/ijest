...
let nowMultipleCallNum = null // 命令行传递的执行次数，优先级高于defaultMultipleCallNum
const utils = {
    assert,
    ...
    test (title, value, asserts, count) {
        ...
        if (typeof value === 'function' && typeof asserts === 'function') {
            return test(title, () => {
                multipleCall(count || nowMultipleCallNum || defaultMultipleCallNum, () => asserts(value()))
            })
        }

        //  test(title, assert) 使用原生的方式进行测试，一般在场景比较复杂的时候使用
        if (typeof value === 'function' && typeof asserts !== 'function') {
            return test(title, value, asserts)
        }
...
function ijest ({ before, after, context = {}, tests = {}, asserts = {}, actives = '', defaultNum = 10 }) {
    ...
    if (typeof before === 'function') {
        beforeAll(() => before(cacheContext))
    }
    if (typeof after === 'function') {
        afterAll(() => after(cacheContext))
    }
...
function startTest (actives) {
    const { actived, a, n } = parseArgv()

    nowMultipleCallNum = n && n.value[0] > 0 ? +n.value[0] : null
